...
void main() {
    ...
    // Pattern 8
    // float strength = floor(mod(vUv.y * 10.0, 1.0) + 0.5); // my approach
    // tutorial approach
    // float strength = mod(vUv.y * 10.0, 1.0);
    ...
    // Pattern 13
    // float strength = step(0.8, mod(vUv.x * 15.0, 1.5));
    // strength *= step(0.8, mod(vUv.y * 15.0, 1.0));
    // gl_FragColor = vec4(strength, strength, strength, 1.0);
    ...
    // Pattern 25
    // float strength = 1.0 - ((1.0 - vUv.x) * (1.0 - vUv.y)); // my approach
    // float strength = length(vUv); // tutorial approach
    // gl_FragColor = vec4(strength, strength, strength, 1.0);

    // // Pattern 26
    // // float strength = length(vUv - 0.5); //approach 1
    // float strength = distance(vUv, vec2(0.5)); //approach 2
    // gl_FragColor = vec4(strength, strength, strength, 1.0);
